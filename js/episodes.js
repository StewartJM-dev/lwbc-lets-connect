/* =========================================================
   LWBC Let's Connect Podcast — episode feed loader
   =========================================================
...
/* =========================================================
   Filtering OUT Question Mark episodes
   =========================================================
   The feed above is the whole "LWBC Let's Connect" channel —
   Pastor Purdy's messages are mixed in with Gordon's Question
   Mark episodes. PodPoint doesn't offer a per-series feed
   (confirmed in Gordon's PodPoint dashboard), so filtering
   happens here. This is the MIRROR of the Question Mark site's
   filter: anything that site would keep, this site drops.

   An episode is treated as Question Mark (and hidden) if EITHER:
   1. Its title is in QUESTION_MARK_TITLES — the frozen list of
      episodes Gordon published before the naming convention
      existed. Copy of the same list the Question Mark site uses;
      keep the two in sync if it ever changes.
   2. Its title starts with the words "Question Mark", any
      punctuation after (colon, dash, em dash, etc.) — the
      going-forward naming convention, so new QM episodes are
      hidden here automatically with no code change.

   If a Question Mark episode ever shows up on this site, it's
   because Gordon titled it without the "Question Mark" prefix —
   either add it to the list below or ask him to retitle it.
   ========================================================= */

var FILTER_ENABLED = true;

var QUESTION_MARK_TITLES = [
...
var QUESTION_MARK_TITLES_NORMALIZED = QUESTION_MARK_TITLES.map(normalizeTitle);

function isQuestionMarkEpisode(title) {
  var norm = normalizeTitle(title);
  if (QUESTION_MARK_TITLES_NORMALIZED.indexOf(norm) !== -1) return true;
  if (/^question mark\b/.test(norm)) return true;
  return false;
}

function fetchLetsConnectEpisodes(callback) {
  fetch(EPISODES_JSON_URL, { cache: "no-store" })
    ...
    .catch(function () {
      fetchLetsConnectEpisodesLive(callback);
    });
}

function fetchLetsConnectEpisodesLive(callback) {
...
function loadEpisodes(opts) {
  ...
  fetchLetsConnectEpisodes(function (episodes) {
...
var PROGRESS_KEY_PREFIX = "lc_progress:";
...
/* =========================================================
   ...
   line, echoing (without copying) the circular crest and
